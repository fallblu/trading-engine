# External strategy protocol v8

Version 8 is a synchronous JSON Lines protocol over child-process standard input and output.
...
Protocol v7 remains available for scenario contract v9; earlier versions retain their frozen
shapes.

Every message repeats `strategy_protocol_version: "8"` and a positive canonical
...
The `initialize` request identifies scenario contract v10 and includes the exact `initial_portfolio`
snapshot alongside the legacy cash projection. It also carries the complete versioned venue
calendars, nested execution configuration, and financing policy, so a strategy can construct DAY
orders and reject incompatible execution or financing state before replay.
...
fills, order updates, and rejected intents. Portfolio contexts include cash-interest attribution.
Response intents use the scenario v10 intent shapes, including recall-origin order snapshots.
