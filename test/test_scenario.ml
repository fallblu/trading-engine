...
let demo_document () =
  In_channel.with_open_bin "../contracts/v7/fixtures/demo.scenario.json"
    In_channel.input_all

let demo () = T.Scenario.of_string (demo_document ()) |> ok
let demo_hash () = T.Sha256.digest_string (demo_document ())
let stream_path = "../contracts/v7/fixtures/demo.scenario.jsonl"
...
let schema_artifacts_parse () =
  ...
  in
  check_schema "../contracts/v7/scenario.schema.json";
  check_schema "../contracts/v7/scenario-stream.schema.json";
  check_schema "../contracts/v7/journal.schema.json"
...
let contract_version_is_required_and_supported () =
  ...
  Alcotest.(check string)
    ...
    "unsupported scenario contract_version \"2\" (expected one of 7, 6, 5, 4, \
     3)"
...
let replay_matches_golden_file () =
  ...
  in
  let expected =
    In_channel.with_open_bin "../contracts/v7/fixtures/demo.journal.jsonl"
      In_channel.input_all
  in
...
let fill_clipping_fixture_reconciles () =
  let document =
    In_channel.with_open_bin
      "../contracts/v7/fixtures/fill-clipped.scenario.json" In_channel.input_all
  in
  let scenario = T.Scenario.of_string document |> ok in
  let result =
    T.Replay.run ~scenario_sha256:(T.Sha256.digest_string document) scenario
  ...
  let expected =
    In_channel.with_open_bin
      "../contracts/v7/fixtures/fill-clipped.journal.jsonl" In_channel.input_all
  in
