...
structural checks. The engine advertises current contract v11 while retaining v10 through v3 and
...
[strategy protocol v9](../contracts/strategy/v9/README.md). Persistra's host turns protocol
...
- **Engine:** `--capabilities` is the authoritative machine-readable surface. The engine must
  ...
  successful run.
- **Scenario:** Frozen scenario and stream artifacts do not change. The current v11 contract may
