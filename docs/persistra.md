...
structural checks. The engine advertises current contract v16 while retaining v15 through v3 and
...
[strategy protocol v14](../contracts/strategy/v14/README.md). Persistra's host turns protocol
...
- **Engine:** `--capabilities` is the authoritative machine-readable surface. The engine must
  ...
  successful run.
- **Scenario:** Frozen scenario and stream artifacts do not change. The current v16 contract may
